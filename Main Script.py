...
EXT_AUDIO = ['mp3', 'mav', 'raw', 'wma', 'mid', 'midi']
...
EXT_INSTL = ['dmg', 'exe', 'ise']

# Step 1 - (Optional) Create Directories where we want to store the files
BASE_PATH = os.path.expanduser('~')
DEST_DIRS = ['Music', 'Movies', 'Pictures', 'Documents', 'Applications', 'Other', 'randomfolderhere']

for d in DEST_DIRS:
    dir_path = os.path.join(BASE_PATH, d)
    if not os.path.join(BASE_PATH, d):
        os.mkdir(dir_path)
...
for file_name in files_list:
    if file_name[0] != '.':
        if file_ext == file_name.split('.')[-1]:
if files_mapping[file_ext].append(file_name):
...
# 10:11pm rn Hoping to get more done
